...
export function PositioningFields({
  ...
  onTargetRolesText: (value: string) => void;
}) {
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <Field
      ...
      <Field label="Target roles (comma separated)" htmlFor="p-roles" className="sm:col-span-2">
        <Input
          id="p-roles"
          value={targetRolesText}
          onChange={(e) => {
            onTargetRolesText(e.target.value);
            patch({ targetRoles: parseComma(e.target.value) });
          }}
...
export function ProjectsFields({
  ...
  return (
    <div className="space-y-4">
      {projects.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Add your real work. 3 real projects beat 30 empty repos. Start with the one that
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Links (comma separated)" htmlFor={`${prefix}-links`}>
          <Input
            ...
            onChange={(e) => onChange({ links: parseComma(e.target.value) })}
